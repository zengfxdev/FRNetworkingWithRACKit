        ...
        if( result == nil || result.count == 0 ){
            ...
            if( code.length > 0 && ![code isEqualToString:@"0"] ){
                ...
                if( [code isEqualToString:@"500"] ){
                    ...
                    [self createServerErrorWithCode:[code integerValue] desc:@"App处理:清空Token" msg:msg];
                }
                else{
...
-(void)createServerErrorWithCode:(NSInteger)code desc:(NSString *)desc msg:(NSString *)msg{
    ...
    [errorlog appendFormat:@"code:%zi >>",code];
    if(msg.length > 0){
        [errorlog appendFormat:@"msg:%@ >>",msg];
    }
    if(desc.length > 0){
        [errorlog appendFormat:@"desc:%@",desc];
    }
    NSDictionary *userInfo = @{NSLocalizedDescriptionKey:errorlog};
    self.error = [NSError errorWithDomain:@"YPServerError" code:code userInfo:userInfo];
}
